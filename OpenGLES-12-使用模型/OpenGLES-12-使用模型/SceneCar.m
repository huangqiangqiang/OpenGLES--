...
@property (assign, nonatomic) GLKVector3 nextPosition;
@property (assign, nonatomic) GLKVector3 velocity;
@property (assign, nonatomic) GLfloat yawRadians;
@property (assign, nonatomic) GLfloat targetYawRadians;
...
- (id)initWithModel:(UtilityModel *)aModel position:(GLKVector3)aPosition velocity:(GLKVector3)aVelocity color:(GLKVector4)aColor
{
    if (self = [super init])
    {
        ...
        self.radius = 0.5f * MAX(axisAlignedBoundingBox.max.x - axisAlignedBoundingBox.min.x,
                                 axisAlignedBoundingBox.max.z - axisAlignedBoundingBox.min.z);
    }
...
- (void)drawWithBaseEffect:(GLKBaseEffect *)anEffect;
{
    ...
    anEffect.transform.modelviewMatrix = GLKMatrix4Translate(savedModelviewMatrix, self.position.x, self.position.y, self.position.z);
